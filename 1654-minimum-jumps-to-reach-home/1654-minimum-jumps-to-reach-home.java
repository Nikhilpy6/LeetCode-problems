class Solution {
  class Pair{
    ...
    public int minimumJumps(int[] forbidden, int a, int b, int x) {
           // start + a
         //start -b
       // min(a,b) and start>=0 and forbidden doesnot contains start
      
      //set to check whether forward move or backward move pos in set or not in  O(1) time
      Set<Integer>visited=new HashSet<>();
        ...
        while(!q.isEmpty()){
            ...
            while(size-->0){
                ...
                if(!isBackward){//if false take backward move
